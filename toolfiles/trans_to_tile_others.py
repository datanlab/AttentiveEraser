...
class InferenceDataset(Dataset):
    ...
    def load_mask(self, mask_path,device='cpu'):
        ...
        mask = F.interpolate(mask, (256, 256))
...
if __name__ == "__main__":
    ...
    inference_dir = '/hy-tmp/gqa_inpaint_inference_6000'
    out_dir = '/hy-tmp/gqa_inpaint_inference_6000'
    os.makedirs(out_dir, exist_ok=True)
    seeds = [123, 321, 777]
    dataset = InferenceDataset(datadir, inference_dir, seeds,img_suffix='.png',inpainted_suffix='_inpainted_')
    ...
    for img_i in tqdm.trange(len(dataset)):
        ...
        save_image(out_1, os.path.join(out_dir, dataset.file_names_seed1[img_i].rsplit('/',1)[1]))
        save_image(out_2, os.path.join(out_dir, dataset.file_names_seed2[img_i].rsplit('/',1)[1]))
        save_image(out_3, os.path.join(out_dir, dataset.file_names_seed3[img_i].rsplit('/',1)[1]))

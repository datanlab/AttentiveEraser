...
if __name__ == "__main__":
    ...
    dataset = InferenceDataset(args.datadir, args.inference_dir, args.test_scene, clip_metric.preprocess, eval_resolution=512, img_suffix='.png', inpainted_suffix='_removed.png')
    ...
    for idx, (source_img, inpainted_img, object_names, scene_id) in enumerate(tqdm(dataloader)):
        ...
        for src_scores, prd_scores, object_name in zip(src_scores_list, prd_scores_list, object_names):
            object_idx = dataset.classes.index(object_name)
            for (c_name, s_score, p_score) in zip(dataset.classes, src_scores, prd_scores):
    ...
    f = open(f"{output_dir}/clip_512.txt", "w")
    f.write("\n".join(out_str_list))
    f.close()